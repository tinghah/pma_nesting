...
const NestingSummary: React.FC<NestingSummaryProps> = ({ result, lang, isDark }) => {
  ...
  const exportToExcel = () => {
    ...
    // Telemetry: Log Export Action
    logUsage('EXPORT_EXCEL', {
        totalQty: result.totalQty,
        orderCount: result.orderTotals.length,
        sizeCount: result.breakdown.length
    });
          ...
          <table className={`min-w-full border-collapse`}>
              ...
              {result.breakdown.map((item) => {
                ...
                return (
                  <tr key={item.size} className={`transition-colors ${isDark ? 'hover:bg-slate-700/50' : 'hover:bg-gray-50'}`}>
                    ...
                    {hasArticle && <td className={`px-4 py-1.5 whitespace-nowrap text-xs font-semibold border ${borderColor} ${articleColor}`}>{item.articles.join('/')}</td>}
                    {hasModel && <td className={`px-4 py-1.5 whitespace-nowrap text-xs font-semibold border ${borderColor} ${modelColor}`}>{item.models.join('/')}</td>}
                    {hasColor && <td className={`px-4 py-1.5 whitespace-nowrap text-xs font-semibold border ${borderColor} ${colorColor}`}>{item.colors.join('/')}</td>}
                    <td className={`px-4 py-1.5 text-sm border ${borderColor}`}>
                        <div className="flex flex-wrap gap-2">
                           {item.orderBreakdown.map((detail, dIdx) => (
                               <div key={dIdx} className={`inline-flex items-center px-1.5 py-0.5 rounded text-[11px] font-medium border ${isDark ? 'bg-slate-700 border-slate-600 text-slate-300' : 'bg-gray-50 border-gray-200 text-gray-700'}`}>
            ...
            <tfoot className={`sticky bottom-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] ${isDark ? 'bg-slate-700' : 'bg-gray-50'}`}>
                <tr>
                    ...
                    {hasArticle && <td className={`px-4 py-2 text-xs font-semibold border ${borderColor} ${isDark ? 'text-slate-300' : 'text-gray-600'}`}>{result.summaryArticles.join(' / ')}</td>}
                    {hasModel && <td className={`px-4 py-2 text-xs font-semibold border ${borderColor} ${isDark ? 'text-slate-300' : 'text-gray-600'}`}>{result.summaryModels.join(' / ')}</td>}
                    {hasColor && <td className={`px-4 py-2 text-xs font-semibold border ${borderColor} ${isDark ? 'text-slate-300' : 'text-gray-600'}`}>{result.summaryColors.join(' / ')}</td>}
